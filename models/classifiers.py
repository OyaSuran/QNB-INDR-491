...
class Classifiers:
    ...
    def plot_scores(self, scores: dict, title: str) -> None:
        ...
        classifiers = [key.name for key in scores["gini"].keys()]
        
        score_gini = [scores["gini"][classifier]    for classifier in scores["gini"].keys()]
        score_roc  = [scores["roc_auc"][classifier] for classifier in scores["roc_auc"].keys()]

        classifiers_axis = np.arange(len(classifiers))

        plt.bar(classifiers_axis - 0.2, score_gini, 0.4, label = 'gini')
        plt.bar(classifiers_axis + 0.2, score_roc,  0.4, label = 'auc')

        for i in range(len(classifiers_axis)):
            plt.text(i - 0.2, score_gini[i] + 0.01, f"{score_gini[i]:.2f}",  ha = 'center')
    ...
    def hyperparams_search(self, n_iter: int, cv: int, exclude: list = []):
        """
        ...
        for classifier in classifers_to_tune:
            ...
            classifier.value.hyperparams_search(self.X_train, self.y_train, n_iter=n_iter, cv=cv)
            result = classifier.value.test(self.X_test, self.y_test)
